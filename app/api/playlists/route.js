...
pool
  .query(
    `ALTER TABLE user_playlists ADD COLUMN IF NOT EXISTS is_public BOOLEAN DEFAULT false;`,
  )
  .catch(() => {});
...
export async function PATCH(req) {
  ...
  try {
    const { playlistId, videoId, action, newSongs, newName, isPublic } =
      await req.json();

    if (action === "visibility" && isPublic !== undefined) {
      await pool.query(
        `UPDATE user_playlists SET is_public = $1 WHERE id = $2 AND user_id = $3`,
        [isPublic, playlistId, session.user.id],
      );
      return NextResponse.json({ success: true });
    }
    ...
    if (action === "rename" && newName) {
      await pool.query(
        `UPDATE user_playlists SET name = $1 WHERE id = $2 AND user_id = $3`,
        [newName, playlistId, session.user.id],
      );
      return NextResponse.json({ success: true });
    }
    ...
    if (res.rows.length === 0)
      return NextResponse.json({ error: "No encontrada" });
