...
const DEFAULT_COVER =
  "https://ui-avatars.com/api/?name=🎵&background=1e1f22&color=a855f7&size=512";
...
export async function POST(req) {
  ...
  try {
    ...
    if (url.includes("spotify.com")) {
      ...
      if (!token)
        return NextResponse.json(
          { error: "Faltan las credenciales de Spotify en el archivo .env" },
          { status: 400 },
        );
      ...
      if (!isPlaylist && !isAlbum)
        return NextResponse.json(
          { error: "Solo se soportan Playlists o Álbumes." },
          { status: 400 },
        );
      ...
      const resInfo = await fetch(endpoint, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!resInfo.ok)
        return NextResponse.json(
          { error: "No se pudo acceder a la playlist de Spotify." },
          { status: 404 },
        );
      ...
      if (!tidalId)
        return NextResponse.json(
          { error: "Enlace de Tidal inválido." },
          { status: 400 },
        );
      ...
      for (const token of publicTokens) {
        try {
          ...
          const infoRes = await fetch(
            `https://api.tidal.com/v1/${type}/${tidalId}?countryCode=US`,
            { headers },
          );
          ...
          if (infoRes.ok) {
            ...
            const itemsRes = await fetch(
              `https://api.tidal.com/v1/${type}/${tidalId}/items?limit=150&countryCode=US`,
              { headers },
            );
      ...
      if (!apiSuccess) {
        ...
        if (!resHtml.ok)
          return NextResponse.json(
            { error: "No se pudo acceder a Tidal." },
            { status: 404 },
          );
          ...
          try {
            ...
            if (data.track && Array.isArray(data.track)) {
              tracks = data.track.map((t) => ({
                ...
                artist: {
                  name:
                    t.byArtist?.name || t.byArtist?.[0]?.name || "Desconocido",
                },
      ...
      if (tracks.length === 0) {
        return NextResponse.json(
          {
            error:
              "No se encontraron canciones. Es posible que el enlace sea privado o inválido.",
          },
          { status: 404 },
        );
      }
      ...
      formattedSongs = tracks.map((track) => {
        ...
        return {
          ...
          artist:
            track.artist?.name || track.artists?.[0]?.name || "Desconocido",
      ...
      return NextResponse.json(
        { error: "Enlace no soportado. Usa Spotify o Tidal." },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json(
      { error: "Error interno al procesar el enlace." },
      { status: 500 },
    );
  }
}
