      ...
      <main className="h-screen bg-transparent text-white flex overflow-hidden font-sans">
        ...
        <aside className="w-[280px] bg-[#0a0a0c]/80 backdrop-blur-xl border-r border-[#1e1f22] flex flex-col pt-8 pb-28 z-10 shadow-[4px_0_24px_rgba(0,0,0,0.5)] hidden md:flex shrink-0">
          <div className="px-4 flex flex-col gap-2 mb-8">
            ...
            <Link href="/playlists" className="flex items-center gap-3 px-4 py-2.5 bg-gradient-to-r from-[#a855f7] to-[#ec4899] text-white rounded-lg font-bold text-sm shadow-[0_0_15px_rgba(168,85,247,0.3)] transition mt-1">
               Tus Playlists
            </Link>
          </div>

          <Link href="/playlists/likes" className="flex items-center gap-4 px-4 py-3 mx-4 mt-2 bg-gradient-to-r from-[#ec4899]/10 to-transparent hover:from-[#ec4899]/20 border border-[#ec4899]/20 hover:border-[#ec4899]/40 rounded-xl transition-all shadow-lg group">
          ...
          <div className="px-4 mt-6 flex flex-col gap-1 flex-1 overflow-y-auto custom-scrollbar">
            ...
            {userPlaylists.map(pl => (
              <Link key={pl.id} href={`/playlists/${pl.id}`} className="flex items-center gap-3 px-4 py-2.5 text-gray-400 hover:text-[#ec4899] hover:bg-white/5 rounded-lg text-sm transition truncate font-medium">
                📁 {pl.name}
              </Link>
            ))}
          </div>
