      ...
      <main className="h-screen bg-transparent text-white flex overflow-hidden font-sans">
        ...
        <aside className="w-[280px] bg-black/20 backdrop-blur-xl border-r border-white/5 flex flex-col pt-8 pb-28 z-10 shadow-[4px_0_30px_rgba(0,0,0,0.5)] hidden md:flex shrink-0 transition-colors duration-1000">
          <div className="px-4 flex flex-col gap-2 mb-8">
            ...
            {/* Botón Home (Inactivo en esta vista) */}
            <Link
              href={`/dashboard/${session.user.id}`}
              className="flex items-center gap-3 px-4 py-2.5 text-gray-400 hover:text-white hover:bg-white/5 rounded-lg font-bold text-sm transition"
            >
              <svg
                className="w-5 h-5"
                fill="none"
            ...
            <Link
              href="/playlists"
              className="flex items-center gap-3 px-4 py-2.5 text-gray-400 hover:text-white hover:bg-white/5 rounded-lg font-bold text-sm transition mt-1"
            >
              <svg
                className="w-5 h-5"
                fill="none"
          ...
          {/* Tarjeta de Tus Favoritos (Activa en esta vista) */}
          <Link
            href="/playlists/likes"
            className="flex items-center gap-4 px-4 py-3 mx-4 mt-2 bg-[var(--dynamic-color-15)] border border-[var(--dynamic-color-30)] rounded-xl transition-all duration-300 shadow-[0_0_15px_var(--dynamic-color-15)] group"
          >
            <div 
              className="w-10 h-10 rounded-lg flex items-center justify-center text-white scale-105 transition-transform duration-300"
              style={{
            ...
            <div className="flex flex-col">
              <span className="text-sm font-bold text-[var(--dynamic-color)] transition-colors duration-300">
                {dict.nav.likes}
              </span>
              <span className="text-[10px] font-medium text-gray-400 transition-colors">
                {likedSongs.length} {dict.playlists.songs}
              </span>
            </div>
          </Link>
