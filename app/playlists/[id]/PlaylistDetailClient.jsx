...
const MusicNoteIcon = () => <svg className="w-12 h-12 text-gray-600 opacity-50" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5"><path strokeLinecap="round" strokeLinejoin="round" d="M9 19V6l12-3v13M9 19c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zm12-3c0 1.105-1.343 2-3 2s-3-.895-3-2 1.343-2 3-2 3 .895 3 2zM9 10l12-3" /></svg>;
const GlobeIcon = () => <svg className="w-3 h-3 inline-block mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9" /></svg>;
const LockIcon = () => <svg className="w-3 h-3 inline-block mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>;
const PlayAllIcon = () => <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>;
const ShareIcon = () => <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" /></svg>;
...
const DragHandleIcon = () => <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path d="M2 5a1 1 0 011-1h14a1 1 0 110 2H3a1 1 0 01-1-1zm0 5a1 1 0 011-1h14a1 1 0 110 2H3a1 1 0 01-1-1zm1 4a1 1 0 100 2h14a1 1 0 100-2H3z"/></svg>;
...
const MosaicCoverPanel = ({ songs }) => {
  if (!songs || songs.length === 0) {
    return <div className="w-32 h-32 md:w-40 md:h-40 bg-white/5 rounded-xl flex items-center justify-center border border-white/10 shadow-inner"><MusicNoteIcon /></div>;
  }
  ...
  if (uniqueCovers.length === 0) return <div className="w-32 h-32 md:w-40 md:h-40 bg-white/5 rounded-xl flex items-center justify-center border border-white/10 shadow-inner"><MusicNoteIcon /></div>;
  if (uniqueCovers.length < 4) return <img src={uniqueCovers[0]} alt="Cover" className="w-32 h-32 md:w-40 md:h-40 rounded-xl object-cover border border-white/10 shadow-lg" />;
        ...
        <div className="p-6 md:p-10 max-w-[1400px] w-full mx-auto flex flex-col gap-6">
          ...
          <div className="glass-panel rounded-3xl p-6 md:p-8 flex flex-col md:flex-row items-center md:items-start gap-6 md:gap-8 shadow-xl">
            ...
            <div className="flex flex-col flex-1 text-center md:text-left w-full justify-center h-full pt-2">
              <span className="text-[10px] font-black uppercase tracking-widest text-[#a855f7] mb-1">
                {isLikesPlaylist ? t('playlists.autoList') : isPublic ? <><GlobeIcon />{t('detail.public')}</> : <><LockIcon />{t('detail.private')}</>}
              </span>
              ...
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 text-sm text-gray-400 font-medium mb-6 bg-white/5 w-fit px-4 py-2 rounded-lg border border-white/5">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-white font-bold">{isOwner ? session.user.name : t('detail.community')}</span>
                </div>
                <span>•</span>
                <span className="font-mono text-xs">{songs.length} {t('playlists.songs')}</span>
              </div>
          ...
          {showSearch && isOwner && !isLikesPlaylist && (
            <div className="glass-panel rounded-2xl p-6 shadow-lg animate-slideDown">
              <h3 className="text-sm font-black uppercase text-white tracking-widest mb-4">{t('detail.quickSearch')}</h3>
              <form onSubmit={handleSearch} className="flex gap-2 mb-4">
                ...
                />
                <button type="submit" disabled={isSearching} className="bg-white/10 hover:bg-white/20 text-white px-6 rounded-xl font-bold transition border border-white/5 min-w-[100px] flex justify-center items-center">
                ...
                <div className="flex flex-col gap-2">
                  {searchResults.map((track, i) => (
                    <div key={i} className="flex items-center justify-between p-3 bg-black/20 border border-white/5 rounded-xl hover:border-[#a855f7]/50 transition-colors group">
                      ...
                      >
                        {isAdding === track.videoId ? <LoadingSpinner /> : t('detail.add')}
                      </button>
                    </div>
                  ))}
                </div>
          ...
          <div className="glass-panel rounded-2xl overflow-hidden shadow-xl">
            ...
            <div className={`grid ${gridColsClass} gap-4 px-6 py-4 border-b border-white/5 bg-black/40 text-[10px] font-black text-[#a855f7] uppercase tracking-widest`}>
              <span className="w-8 text-center cursor-pointer hover:text-white transition-colors" onClick={() => handleSort('default')}>
                # {sortConfig.key === 'default' ? '↓' : ''}
              </span>
              
              <div className="flex items-center gap-4">
                  <span className="cursor-pointer hover:text-white transition-colors flex items-center gap-1" onClick={() => handleSort('title')}>
                    {t('detail.table.title')} {sortConfig.key === 'title' ? (sortConfig.direction === 'asc' ? '↑' : '↓') : ''}
                  </span>
                  <span className="text-gray-600">/</span>
                  <span className="cursor-pointer hover:text-white transition-colors flex items-center gap-1" onClick={() => handleSort('artist')}>
                    {t('detail.table.artist')} {sortConfig.key === 'artist' ? (sortConfig.direction === 'asc' ? '↑' : '↓') : ''}
                  </span>
              </div>
              
              {!isLikesPlaylist && (
                <span className="hidden md:flex cursor-pointer hover:text-white transition-colors items-center gap-1" onClick={() => handleSort('requester')}>
                  {t('detail.table.addedBy')} {sortConfig.key === 'requester' ? (sortConfig.direction === 'asc' ? '↑' : '↓') : ''}
                </span>
              )}
            ...
            {isMounted && (
              <DragDropContext onDragEnd={handleDragEnd}>
                <Droppable droppableId="playlist-songs" isDropDisabled={!canDrag}>
                  {(provided) => (
                    <div {...provided.droppableProps} ref={provided.innerRef} className="flex flex-col p-2 min-h-[100px]">
                      {sortedSongs.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
                          <p className="text-gray-500 font-medium mb-4">{t('detail.table.empty')}</p>
                          {isOwner && !isLikesPlaylist && (
                          ...
                          return (
                            <Draggable key={`${song.videoId}-${index}`} draggableId={`${song.videoId}-${index}`} index={index} isDragDisabled={!canDrag}>
                              {(provided, snapshot) => (
                                ...
                                >
                                  {/* Grip Handle */}
                                  <div {...provided.dragHandleProps} className={`w-8 h-8 flex items-center justify-center text-gray-500 rounded transition-colors ${canDrag ? 'hover:bg-white/10 hover:text-white cursor-grab' : ''}`}>
                                  ...
                                  <div className="flex items-center gap-4 min-w-0">
                                    {hasValidCover ? (
                                      <img src={song.thumbnail} className="w-10 h-10 object-cover rounded-md shadow-sm border border-white/5" alt="cover" />
                                    ) : (
                                  ...
                                  {!isLikesPlaylist && (
                                    <div className="hidden md:flex items-center gap-2 min-w-0">
                                      {song.requesterAvatar ? (
                                        <img src={song.requesterAvatar} className="w-5 h-5 rounded-full border border-white/10" alt="avatar" />
                                      ) : (
            ...
            <div className="glass-panel rounded-2xl p-6 shadow-sm">
              <div className="flex justify-between items-center mb-6">
                <div className="flex flex-col">
                  <h3 className="text-sm font-black uppercase text-white tracking-widest">{t('dashboard.recommendations.title')}</h3>
                ...
                >
                  {isFetchingRecs ? <LoadingSpinner /> : <><RefreshIcon /> {t('dashboard.recommendations.refresh')}</>}
                </button>
              </div>

              {recommendations.length === 0 && !isFetchingRecs ? (
                <p className="text-xs text-gray-500 italic text-center py-4">{t('dashboard.recommendations.empty')}</p>
              ) : (
                <div className="flex flex-col gap-2">
      ...
      {isEditModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fadeIn">
          <div className="glass-panel rounded-3xl p-8 w-full max-w-md shadow-[0_0_50px_rgba(168,85,247,0.15)] border border-[#a855f7]/30 animate-slideUp">
            ...
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-black text-white">{t('detail.settings.title')}</h2>
              <button onClick={() => setIsEditModalOpen(false)} className="text-gray-500 hover:text-white transition"><TrashIcon className="w-6 h-6 rotate-45" /></button>
            </div>

            <div className="flex flex-col gap-6">
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-2 block">{t('detail.settings.nameLabel')}</label>
                <input 
                  type="text" 
              ...
              <div className="flex items-center justify-between bg-white/5 p-4 rounded-xl border border-white/10">
                <div className="flex flex-col">
                  <span className="text-sm font-bold text-white">{t('detail.settings.publicLabel')}</span>
                  <span className="text-xs text-gray-400">{t('detail.settings.publicDesc')}</span>
                </div>
              ...
              <div className="p-4 border border-[#7e22ce]/30 bg-[#7e22ce]/10 rounded-xl flex flex-col items-start gap-3 mt-2">
                <span className="text-sm font-bold text-[#7e22ce] uppercase tracking-widest">{t('detail.settings.dangerZone')}</span>
                <span className="text-xs text-gray-400">{t('detail.settings.dangerDesc')}</span>
                <button 
                ...
                >
                  {isDeleting ? <LoadingSpinner /> : t('detail.settings.deleteBtn')}
                </button>
              </div>
            </div>

            <div className="flex justify-end gap-3 mt-8">
              <button onClick={() => setIsEditModalOpen(false)} className="px-5 py-3 rounded-full font-bold text-sm text-gray-400 hover:text-white transition">{t('common.cancel')}</button>
              <button 
              ...
              >
                {isSavingEdit ? <LoadingSpinner /> : t('detail.settings.saveBtn')}
              </button>
            </div>
