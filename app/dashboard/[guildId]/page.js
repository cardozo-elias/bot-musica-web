...
export default async function DashboardPage({ params }) {
  ...
  try {
    ...
    if (userStatsRes.rows.length > 0) {
      stats.listenTimeHours = (userStatsRes.rows[0].listen_time / 3600).toFixed(
        1,
      );
    ...
    const plRes = await pool.query(
      "SELECT id, name FROM user_playlists WHERE user_id = $1",
      [session.user.id],
    );
    ...
    try {
      const historyRes = await pool.query(
        'SELECT title, artist, video_id as "videoId", played_at as "playedAt" FROM user_history WHERE user_id = $1 ORDER BY played_at DESC LIMIT 30',
        [session.user.id],
      );
      ...
      <main className="h-screen bg-transparent text-white flex overflow-hidden font-sans">
        ...
        <aside className="w-[280px] bg-black/20 backdrop-blur-xl border-r border-white/5 flex flex-col pt-8 pb-28 z-10 shadow-[4px_0_30px_rgba(0,0,0,0.5)] hidden md:flex shrink-0 transition-colors duration-1000">
          <div className="px-4 flex flex-col gap-2 mb-8">
            <div className="px-4 py-1 text-[10px] font-black uppercase text-gray-500 tracking-widest mb-2">
              Navegación
            </div>

            {/* Botón Home adaptado al color dinámico */}
            <button 
              className="flex items-center gap-3 px-4 py-2.5 text-white rounded-lg font-bold text-sm shadow-lg hover:scale-[1.02] hover:brightness-110 transition-all duration-300"
              style={{ 
          ...
          {/* Tarjeta de Tus Favoritos Dinámica */}
          <Link
