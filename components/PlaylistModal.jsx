    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
      <div className="bg-[#1e1f22] border border-[#2b2d31] w-full max-w-md rounded-xl shadow-2xl overflow-hidden">
        ...
        <div className="p-2 max-h-60 overflow-y-auto">
          {loading ? (
            <p className="p-4 text-center text-gray-500">Cargando tus listas...</p>
          ) : playlists.length === 0 ? (
            <p className="p-4 text-center text-gray-500 text-sm">No tienes playlists creadas.</p>
          ) : (
            playlists.map((pl) => (
              <button
                key={pl.id}
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-[10px] text-gray-500 uppercase font-bold">{pl.songs?.length || 0} canciones</p>
                </div>
              </button>
            ))
          )}
