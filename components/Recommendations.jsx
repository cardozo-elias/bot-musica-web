  ...
  if (loading)
    return (
      <div className="glass-panel p-6 rounded-3xl border border-white/5 animate-pulse w-full h-[550px]">
        ...
        {[1, 2, 3, 4, 5].map((i) => (
          <div key={i} className="h-14 bg-black/20 rounded-xl mb-3"></div>
        ))}
      </div>
    );
        ...
        {recs.map((track) => (
          ...
          >
            <div className="relative h-12 w-12 flex-shrink-0">
              ...
              <div
                className={`absolute inset-0 bg-black/60 flex items-center justify-center transition rounded-xl ${loadingTrackId === track.videoId ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}
              >
