    ...
    socketRef.current.on("like_added", (data) => {
      if (data.userId === userId) {
        setLikes((prev) => {
          ...
          return [
            { videoId: data.videoId, title: data.title, artist: data.artist },
            ...prev,
          ];
        });
      }
  ...
  return (
    <div className="px-4 flex flex-col gap-1">
      ...
      <button
        ...
        className="px-4 py-2 flex justify-between items-center w-full hover:bg-[#1e1f22]/50 rounded-md transition group"
      >
      ...
      {isOpen && (
        <div className="max-h-[380px] overflow-y-auto custom-scrollbar flex flex-col gap-1 pr-1 mt-1 animate-fadeIn relative">
          {likes.length === 0 ? (
            <p className="px-4 text-xs text-gray-600 mt-1 italic">
              No hay favoritos.
            </p>
          ) : (
            likes.map((like) => (
