...
export default function RecentlyPlayed({ history: initialHistory, userId, userName, userAvatar }) {
  ...
  const handlePlay = (song) => {
    ...
    socketRef.current?.emit("cmd_play_specific", { userId, video: { videoId: song.videoId, title: song.title, author: song.artist }, userName, userAvatar });
    ...
    <div className="w-full glass-panel p-6 md:p-8 rounded-3xl border border-white/5 shadow-lg flex flex-col h-[550px]">
      <div className="flex justify-between items-center mb-6 border-b border-white/5 pb-4">
          <div className="flex flex-col">
            <h3 className="text-xl md:text-2xl font-black text-white tracking-tighter">{t('dashboard.recent.title')}</h3>
            <span className="text-xs text-gray-500 font-medium">{t('dashboard.recent.subtitle')}</span>
          </div>
      </div>
      
      {history.length === 0 ? (
        <div className="flex flex-col items-center justify-center flex-1 text-center opacity-50">
            <p className="text-gray-400 font-bold mb-2">{t('dashboard.recent.empty')}</p>
        </div>
      ) : (
        <div className="flex flex-col gap-2 overflow-y-auto custom-scrollbar pr-2 flex-1">
          {history.map((song, index) => (
            <div key={`${song.videoId}-${index}`} className="group flex items-center justify-between bg-black/20 p-3 rounded-2xl hover:bg-black/40 transition border border-transparent hover:border-[#a855f7]/30">
              <div className="flex items-center gap-4 overflow-hidden">
                <span className="text-gray-600 font-black text-xs w-8 text-center group-hover:text-[#a855f7] transition-colors"><MusicIcon /></span>
                <div className="flex flex-col min-w-0">
                  <span className="font-bold text-white text-sm truncate group-hover:text-[#a855f7] transition cursor-default">{song.title}</span>
              ...
              <div>
                ...
                >
                  {loadingTrackId === song.videoId ? <LoadingSpinner /> : t('player.play')}
                </button>
              </div>
            </div>
          ))}
